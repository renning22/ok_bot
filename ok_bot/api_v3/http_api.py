...
class OKHttpV3:
    ...
    def create_order(self, custom_order_id, instrument_id, type, amount, price, leverage=10):
        """
        :param instrument_id: for example: "TC-USD-180213"
        :param type: 1:open long 2:open short 3:close long 4:close short
        :param side:
        :param amount:
        :param price:
        :param custom_order_id:
        :param leverage: 10x or 20x
        ...
        * price, amount etc can be int or str, they are all converted to string before being used to compose the
        ...
        amount = int(amount)
        return self.future_sdk.take_order(
            custom_order_id,
            instrument_id,
            type,
            price,
...
if __name__ == '__main__':
    ...
    if args.action == 'cancel_order':
        ...
        func(args.instrument_id, args.order_id)
    else:
        print(f'action:{args.action}, {args.instrument_id} {args.price}@{args.volume}, {args.custom_id}')
        func(args.instrument_id, args.volume, args.price, args.custom_id)
